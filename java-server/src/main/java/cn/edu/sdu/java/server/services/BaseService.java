...
public class BaseService {
    ...
    public MyTreeNode getMaterialTreeNode() {
        ...
        File materialDir = new File(attachFolder, "material");
        if (materialDir.exists() && materialDir.isDirectory()) {
            buildMaterialTree(root, materialDir);
        }
        return root;
    }

    private void buildMaterialTree(MyTreeNode parentNode, File dir) {
        ...
        for (File file : files) {
            ...
            node.setId(parentNode.getId() * 1000 + id++);
            node.setLabel(file.getName());
            node.setValue(file.getName());
            node.setPid(parentNode.getId());

            if (file.isDirectory()) {
                node.setIsLeaf(0);
                node.setChildren(new ArrayList<>());
                buildMaterialTree(node, file);
            } else {
                node.setIsLeaf(1);
            }
