...
public class WelcomeTeacherController {
    ...
    @FXML
    protected void onMyCourseClick() {
        MainFrameController mainFrame = AppStore.getMainFrameController();
        if (mainFrame != null) {
            mainFrame.changeContent("TeacherCoursePanel", "我的课程");
        }
    }

    @FXML
    protected void onScoreEntryClick() {
        MainFrameController mainFrame = AppStore.getMainFrameController();
        if (mainFrame != null) {
            mainFrame.changeContent("TeacherScorePanel", "成绩录入");
        }
    }

    @FXML
    protected void onAttendanceManageClick() {
        MainFrameController mainFrame = AppStore.getMainFrameController();
        if (mainFrame != null) {
            mainFrame.changeContent("TeacherAttendancePanel", "考勤管理");
        }
    }
}
